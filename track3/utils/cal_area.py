...
save_root_dir = 'home_directory/VIC/track3/new/filtered_mot_res'
...
for i in range(len(all_locs)):
    ...
    # pdb.set_trace()
    # area.sort()
    # roi_area = area[int(num * 0.6):]

    ind = np.where(area > 18000)[0]
    # print all_locs[i], 'total:', len(area), '0.6:', roi_area[0] #, 'ratio:', len(ind) / float(len(area))
    print all_locs[i], 'total:', len(area), '0.6:', 'ratio:', len(ind) / float(len(area))
    # for t in thresh:
